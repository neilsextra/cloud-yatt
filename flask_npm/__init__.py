...
def serve(component, filename):
    ...
    root = current_app.config['NPM_COMPONENTS_ROOT']
 
    return send_file(os.path.join(root, component, filename), conditional=True) 

def npm_url_for(component, filename, **values):
    """
    DEPRECATED
    This function provides backward compatibility - please migrate to the approach using "npm.static"

    :param component: npm component (package)
    :type component: str

    :param filename: filename in npm component - can contain directories (like dist/jquery.js)
    :type filename: str
...
def replaced_url_for(endpoint, filename=None, **values):
    """
    This function acts as "replacement" for the default url_for() and intercepts if it is a request for npm assets
    If the file is not available in npm, the result is passed to flasks url_for().
...
def overlay_url_for(endpoint, filename=None, **values):
    ...
    If the requested endpoint is static or ending in .static, it tries to serve a npm asset, otherwise it will pass
...
def build_url(component, filename, **values):
    """
    search npm asset and build url
    ...
    # check if component exists in npm_components directory
    ...
    if current_app.config['NPM_TRY_MINIFIED']:
    ...
    if current_app.config['NPM_QUERYSTRING_REVVING']:

        if npm_data is not None and 'version' in npm_data:
            values['version'] = npm_data['version']
    ...
    return url_for('npm.serve', component=component, filename=filename, **values)
...
class Npm(object):
    ...
    def init_app(self, app):
        app.config.setdefault('NPM_COMPONENTS_ROOT', 'node_modules')
        app.config.setdefault('NPM_KEEP_DEPRECATED', True)
        app.config.setdefault('NPM_QUERYSTRING_REVVING', True)
        app.config.setdefault('NPM_REPLACE_URL_FOR', False)
        app.config.setdefault('NPM_SUBDOMAIN', None)
        app.config.setdefault('NPM_TRY_MINIFIED', True)
        app.config.setdefault('NPM_URL_PREFIX', '/npm')
 
        blueprint = Blueprint(
            'npm',
            __name__,
            url_prefix=app.config['NPM_URL_PREFIX'],
            subdomain=app.config['NPM_SUBDOMAIN'])

        blueprint.add_url_rule('/<component>/<path:filename>', 'serve', serve)

        app.register_blueprint(blueprint)

        if app.config['NPM_KEEP_DEPRECATED'] is True:
            app.jinja_env.globals['npm_url_for'] = npm_url_for

        if app.config['NPM_REPLACE_URL_FOR'] is True:
